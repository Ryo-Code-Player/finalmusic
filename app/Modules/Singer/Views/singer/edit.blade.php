...
<div class="grid grid-cols-12 gap-12 mt-5">
    <div class="intro-y col-span-12 lg:col-span-12">
        <form method="post" action="{{ route('admin.singer.update', $singer->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="intro-y box p-5">
                <div class="mt-3">
                    <label for="alias" class="form-label">Tên Ca Sĩ</label>
                    <input id="alias" name="alias" type="text" class="form-control" placeholder="Tên Ca Sĩ" value="{{ old('alias', $singer->alias) }}" required>
                </div>
                ...
                <div class="mt-3">
                    ...
                    <div class="px-4 pb-4 mt-5 flex items-center cursor-pointer relative">
                        ...
                        <input type="hidden" id="photo" name="photo" value="{{ old('photo', $singer->photo) }}"/>
                    </div>
                </div>
                <div class="form-group">
    <label for="start_year">Chọn Năm Bắt Đầu</label>
    <input type="text" name="start_year" id="start_year" class="form-control" placeholder="Nhập năm bắt đầu" value="{{ old('start_year', $singer->start_year ?? date('Y')) }}">
</div>
                ...
                <div class="form-group">
                    ...
                    <select name="type_id" id="music_type" class="form-control" required>
                        ...
                        @foreach($music_types as $musicType)
                            <option value="{{ $musicType->id }}" {{ $musicType->id == $singer->type_id ? 'selected' : '' }}>{{ $musicType->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="company_id" id="company_id" class="form-control" required>
                        ...
                        @foreach($musicCompanies as $company)
                            <option value="{{ $company->id }}" {{ $company->id == $singer->company_id ? 'selected' : '' }}>{{ $company->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mt-3">
                    ...
                    <textarea class="form-control" id="editor1" name="summary">{{ old('summary', $singer->summary) }}</textarea>
                        ...
                        <textarea class="form-control" name="content" id="editor2">{{ old('content', $singer->content) }}</textarea>
                    </div>
...
<input type="hidden" name="new_tags_input" id="new_tags_input" value="{{ old('new_tags_input') }}">
                ...
                <div class="mt-3">
                    ...
                    <select name="status" class="form-select mt-2" required>
                        <option value="active" {{ $singer->status == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ $singer->status == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
