...
<div class="content">
    <h2 class="intro-y text-lg font-medium mt-10">
        Kết quả tìm kiếm thể loại Âm nhạc
    </h2>
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <table class="table table-report -mt-2" id="musicTypeTable">
            <thead>
                <tr>
...
<script>
    ...
    $("#search").on('keyup', function() {
        ...
        $("#musicTypeTable tbody tr").filter(function() {
            $(this).toggle($(this).find('.music-type-title').text().toLowerCase().indexOf(value) > -1);
        });
    });

    $("[name='toogle']").change(function() {
        var mode = $(this).prop('checked') ? 'active' : 'inactive'; // Xác định trạng thái
        var id = $(this).val(); // ID của thể loại âm nhạc

        // Đảm bảo URL đúng, truyền id vào URL
        $.ajax({
            url: "{{ route('admin.musictype.status', ['id' => '__id__']) }}".replace('__id__', id),
            type: "POST",
            data: {
                _token: '{{ csrf_token() }}',
