...
<div class="blog-form-container">
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('front.blog.update') }}">
        @csrf
        <input type="hidden" name="id" value="{{ $blog->id }}">
        <div class="form-group">
            <label class="form-label" for="title">Tiêu đề</label>
            <input type="text" class="form-control" id="title" name="title" placeholder="Nhập tiêu đề bài viết" value="{{ $blog->title }}" required>
        </div>
        <div class="form-group">
            <label class="form-label">Photo</label>
            <div id="preview-photo" style="margin-top: 10px;">
                <img src="{{ asset($blog->photo) }}" alt="Blog 1">
            </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="description" name="description" placeholder="Nhập mô tả ngắn" value="{{ $blog->summary }}" required>
        </div>
        <div class="form-group">
            <label class="form-label" for="content">Nội dung</label>
            <textarea class="form-control" id="content" name="content" rows="7" placeholder="Nhập nội dung bài viết" value="{{ $blog->content }}">{{ $blog->content }}</textarea>
        </div>
        <div class="form-group">
            ...
            <select class="form-select" id="category" name="category" required>
                ...
                @foreach($blogCategories as $blogCategory)
                    <option value="{{ $blogCategory->id }}" {{ $blog->cat_id == $blogCategory->id ? 'selected' : '' }}>{{ $blogCategory->title }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-select" id="status" name="status" required>
                <option value="active" {{ $blog->status == 'active' ? 'selected' : '' }}>Hoạt động</option>
                <option value="inactive" {{ $blog->status == 'inactive' ? 'selected' : '' }}>Không hoạt động</option>
            </select>
        </div>
